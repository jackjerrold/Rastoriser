...
def DrawCube(img, points,color, fill):
    width = points[1].x - points[0].x
    height = points[1].y - points[0].y
    depth = points[1].z - points[0].z

    p1 = points[0]
    p2 = vectors.Vector3(points[0].x,points[0].y+height,points[0].z)
    p3 = vectors.Vector3(points[0].x + width,points[0].y,points[0].z)
    DrawTri(img, [p1.DimensionDown(),p2.DimensionDown(),p3.DimensionDown()],color,fill)

    p1 = vectors.Vector3(points[0].x+width,points[0].y+height,points[0].z)
    p2 = vectors.Vector3(points[0].x,points[0].y+height,points[0].z)
    p3 = vectors.Vector3(points[0].x + width,points[0].y,points[0].z)
    DrawTri(img, [p1.DimensionDown(),p2.DimensionDown(),p3.DimensionDown()],color,fill)

    p1 = vectors.Vector3(points[0].x+width +2,points[0].y+height,points[0].z+depth)
    p2 = vectors.Vector3(points[0].x+width,points[0].y,points[0].z+depth)
    p3 = vectors.Vector3(points[0].x+width,points[0].y,points[0].z)
    DrawTri(img, [p1.DimensionDown(),p2.DimensionDown(),p3.DimensionDown()],color,fill)
...
p1 = vectors.Vector3(-30,-30,1)
p2 = vectors.Vector3(30,30,60)
DrawCube(img,[p1,p2],WHITE,False)
SaveBitmap(img, "3DRender.png")
print("Complete.")